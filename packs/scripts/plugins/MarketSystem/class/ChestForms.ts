...
import { CHEST_UI_SIZES, custom_content, custom_content_keys, inventory_enabled, number_of_custom_items } from "../utils/Constants.ts";
...
class ChestFormData {
    ...
    private buttonArray: ([RawText, string])[];
	...
	title(text: string | RawText): ChestFormData {
	...
	button(slot: number, itemName: string | RawText, itemDesc: (string | RawText)[], texture: string, stackSize = 1, durability = 0, enchanted = false) {
		const targetTexture = custom_content_keys.has(texture) ? custom_content[texture as keyof typeof custom_content]?.texture : texture;
		const ID = typeIdToDataId.get(targetTexture) ?? typeIdToID.get(targetTexture);
		const buttonRawtext: RawText = {
			rawtext: [
				{
		...
		this.buttonArray.splice(Math.max(0, Math.min(slot, this.slotCount - 1)), 1, [
			buttonRawtext,
			(ID === undefined ? targetTexture : ((ID + (ID < 256 ? 0 : number_of_custom_items)) * 65536) + (enchanted ? 32768 : 0)) as string
		]);
		return this;
	}
	pattern(pattern: string[], key: Record<string, { texture: string; stackAmount?: number; durability?: number; itemName: string | RawText; itemDesc?: (string | RawText)[]; enchanted?: boolean }>): ChestFormData {
